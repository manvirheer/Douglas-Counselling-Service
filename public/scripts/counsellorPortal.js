...
auth.onAuthStateChanged(user => {
    if (user) {
                        ...
                        db.collection("admin-availability").onSnapshot(snapshot => {
                            snapshot.forEach(docInner => {
                                if (docInner.id == doc.data().admin_availability) {
                                    ...
                                    db.collection('students').doc(doc.data().student_id).get().then(docSpec => {
                                        ...
                                        window.localStorage.setItem("roomIDGlobal",doc.data().admin_availability);
                                        var varr = user.uid;
                                        window.localStorage.setItem("userID",varr);
                                        var studID = doc.data().student_id;
                                        window.localStorage.setItem("roomEmailStudentGlobal",studID);
                                        console.log("sid "+studID+"aid "+user.uid+"mid "+doc.id);
                                        table.innerHTML += `<tr>
                        <td>${type}</td>
                        <td>
        ...
        timeForm.addEventListener("submit", function(e){
            ...
            console.log("hey");
            db.collection("admin-availability").add({
               admin_id : user.uid,
               "type": type,
