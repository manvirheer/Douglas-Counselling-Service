                            ...
                            snapshot.forEach(docInner => {
                                if ((docInner.id == doc.data().admin_availability)){
                                    ...
                                    db.collection('admins').doc(docInner.data().admin_id).get().then(docSpec => {
                                        ...
                                        window.localStorage.setItem("roomEmailStudentGlobal",user.uid);
                                        table.innerHTML += `<tr>
                                            <td>${type}</td>
                                            <td>
                                            ...
                                            <td>Dr. ${name}</td>
                                            <td><a href="createChat.html?studentID=${user.uid}&adminID=${docSpec.id}&meetingID=${doc.id}">BUTTON<a></td>
                                            </tr>`;
